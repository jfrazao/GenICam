...
- `NumBuffers` — size of the fixed GenTL buffer pool (default `4`). Each frame is copied into its own `IplImage` and the buffer is re-queued immediately, so holding images downstream never starves the pool; but if a downstream node *blocks* the acquisition thread, frames are dropped once all buffers fill. Raise this to tolerate longer processing bursts. See the Architecture article ("Frame buffer lifecycle and backpressure").
